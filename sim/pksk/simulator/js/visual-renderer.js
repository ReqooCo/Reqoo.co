...
function coordinateMove(v){
  ...
  return svg(`<rect x="18" y="15" width="524" height="270" rx="18" fill="${COLORS.paper}" stroke="${COLORS.line}"/>${grid}${line(ox,oy,ox+225,oy,'stroke="'+COLORS.navy+'"')}${line(ox,oy,ox,25,'stroke="'+COLORS.navy+'"')}<circle cx="${x1}" cy="${y1}" r="7" fill="${COLORS.navy}"/>${text(x1,y1-15,`(${sx}, ${sy})`,'font-size="13"')}<line x1="${x1+10}" y1="${y1}" x2="${x2-10}" y2="${y2}" stroke="${COLORS.teal}" stroke-width="4" marker-end="url(#arrow)"/><defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="5" refY="3" orient="auto"><path d="M0,0 L0,6 L6,3 z" fill="${COLORS.teal}"/></marker></defs><circle cx="${x2}" cy="${y2}" r="7" fill="#fff" stroke="${COLORS.teal}" stroke-width="3"/>${text((x1+x2)/2,y1-18,`${move} unit ke kanan`,'font-size="13" fill="'+COLORS.teal+'"')}${text(x2,y2+27,'?','font-size="17" fill="'+COLORS.teal+'"')}`,'Rajah koordinat dan pergerakan ke kanan','0 0 560 300');
}
