...
window.playAnnouncement=function(key,after){
  ...
  if(h)h.textContent=b.title;if(p)p.textContent=b.text;if(s)s.textContent='Arahan dipaparkan pada skrin. Tiada audio digunakan.';if(mic)mic.style.display='none';
...
window.countdown=function(callback){
  const box=$('count');if(!box)return typeof callback==='function'&&callback();
  box.innerHTML='';const b=document.createElement('button');b.type='button';b.className='v59-start-btn';
  b.textContent=(document.getElementById('briefingTitle')?.textContent||'').includes('Bahagian C')?'MULA BAHAGIAN C →':'MULA BAHAGIAN A + B →';
  b.onclick=function(){b.disabled=true;if(typeof callback==='function')callback()};box.appendChild(b);
};
