...
function finishSyncAndGo(btn){const code=L(),n=setNo();let st=null;try{st=JSON.parse(localStorage.getItem(oldKey(n))||'null')}catch(_){}const answers=st?.answers||{},essay=String(st?.essay||'');const done=()=>go();if(!code||!dev()||!Object.keys(answers).length){setTimeout(done,1800);return}btn.disabled=true;btn.textContent='MENYIMPAN…';jsonp('saveProgress',{code,deviceId:dev(),setNo:n,section:'OVERALL',completed:true,score:0,answered:Object.keys(answers).length,timeUsed:Math.max(0,Math.round((5400-Number(st?.timer||5400)+2700-Number(st?.wTimer||2700))/60)),answers:JSON.stringify(answers),essayText:essay,essayWords:essay.trim().split(/\s+/).filter(Boolean).length,completedAt:new Date().toISOString()},()=>setTimeout(done,500))}
