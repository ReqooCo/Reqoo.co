(function(){
  ...
  function call(action,data,done,retries=2){
    const attempt=n=>{const cb='pkskV56_'+Date.now()+'_'+Math.floor(Math.random()*99999),sc=document.createElement('script');let finished=false;const timer=setTimeout(()=>finish({ok:false,error:'Server mengambil masa terlalu lama.'}),11000);function finish(r){if(finished)return;finished=true;clearTimeout(timer);delete window[cb];sc.remove();if(r&&r.ok!==false){done(r);return}if(n<retries){setTimeout(()=>attempt(n+1),700*(n+1));return}done(r||{ok:false,error:'Sambungan server gagal.'})}window[cb]=finish;sc.onerror=()=>finish({ok:false,error:'Sambungan server gagal.'});sc.src=V56+'?'+new URLSearchParams({action,callback:cb,...data});document.body.appendChild(sc)};attempt(0)
  }
  ...
  if(typeof window.syncProgressServer==='function'&&!window.syncProgressServer.__v56){
    const fn=function(score,extra){
      ...
      let answerMap={};try{answerMap=answers||{}}catch(_){}
      let currentSet=1;try{currentSet=Number(setNo||1)}catch(_){}
      const payload=Object.assign({code,deviceId:typeof pkskDeviceId==='function'?pkskDeviceId():'',setNo:currentSet,completed:true,score:Number(score||0),answers:JSON.stringify(answerMap)},extra||{});
