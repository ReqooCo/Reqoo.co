...
function render(){if(!state.started)return;const q=state.items[state.index];const pct=Math.round((state.index/state.items.length)*100);$('#progress').style.width=pct+'%';$('#count').textContent=`${state.index+1} / ${state.items.length}`;$('#section').textContent=q.section||'';$('#category').textContent=q.category||'';$('#qid').textContent=q.id||'';$('#question').textContent=q.question||'';const chosen=state.answers[state.index];$('#options').innerHTML=(q.options||[]).map((o,i)=>`<button class="option ${chosen===i?'selected':''}" data-i="${i}">${esc(o)}</button>`).join('');$('#back').disabled=state.index===0;$('#next').textContent=state.index===state.items.length-1?'Tamat & Semak':'Seterusnya';$('#quiz').classList.remove('hidden');$('#result').classList.add('hidden');$('#writing').classList.add('hidden')}
function finish(){let total=0,max=0;state.items.forEach((q,i)=>{const w=q.weights||[];max+=Math.max(...w,0);if(state.answers[i]!==undefined)total+=Number(w[state.answers[i]]||0)});state.score=total;$('#score').textContent=`${total} / ${max}`;$('#resultText').textContent=`Set ${state.data.set} selesai. Jawapan telah dikira berdasarkan skor item.`;$('#quiz').classList.add('hidden');$('#result').classList.remove('hidden');$('#writing').classList.add('hidden');$('#writingList').innerHTML=(state.data.writing||[]).map((w,i)=>`<button class="option" data-writing="${i}"><strong>${esc(w.title)}</strong><br><span class="muted">Minimum ${w.min_words||0} perkataan</span></button>`).join('')}
...
const select=$('#set');for(let i=1;i<=50;i++){const o=document.createElement('option');o.value=i;o.textContent='Set '+String(i).padStart(2,'0');select.appendChild(o)}select.value=1;loadSet(1);
