...
function rewrite(){
  ...
  if(!document.getElementById('reqoo-landing-trust')){
    ...
    if(hero){const bar=document.createElement('div');bar.id='reqoo-landing-trust';bar.innerHTML='<div><strong>Rekaan diperibadi</strong><small>Nama, logo & cerita anda</small></div><div><strong>Semakan sebelum produksi</strong><small>Butiran disahkan bersama</small></div><div><strong>Harga lebih jelas</strong><small>Pilihan dipaparkan di Shop</small></div><div><strong>Sokongan terus</strong><small>Bincang bersama pasukan Reqoo</small></div>';bar.style.cssText='width:min(1200px,calc(100% - 48px));margin:20px auto 0;display:grid;grid-template-columns:repeat(4,1fr);gap:8px;color:#bbb;font-size:11px';hero.appendChild(bar)}}
