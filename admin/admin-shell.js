...
async function logout(){try{await fetch('/api/admin-session',{method:'DELETE',cache:'no-store'})}catch{}localStorage.removeItem('reqoo_admin_token');localStorage.removeItem('REQOO_ADMIN_TOKEN');localStorage.removeItem('reqoo_shop_admin_token');document.cookie='reqoo_admin_token=; Domain=.reqoo.co; Path=/; Max-Age=0; Secure; SameSite=Lax';document.cookie='reqoo_admin_token=; Path=/; Max-Age=0; Secure; SameSite=Lax';sessionStorage.removeItem('reqoo_admin_return');location.replace('/admin/')}document.querySelectorAll('[data-rq-logout]').forEach(b=>b.addEventListener('click',logout));
