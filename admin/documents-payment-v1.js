...
function ensureModal(){
 ...
 const closePay=()=>{$('#rqPaymentModal').classList.remove('open');$('#rqPaymentModal').setAttribute('aria-hidden','true')};
 const closeRc=()=>{$('#rqReceiptModal').classList.remove('open');$('#rqReceiptModal').setAttribute('aria-hidden','true')};
 $('#rqPayClose').onclick=closePay;$('#rqPayCancel').onclick=closePay;$('#rqReceiptClose').onclick=closeRc;$('#rqReceiptCloseBottom').onclick=closeRc;$('#rqPaySave').onclick=savePayment;$('#rqReceiptPrint').onclick=printReceipt;
}
...
function injectPaymentReceipts(){
 ...
 payments.slice().reverse().forEach(p=>{
  if(host.querySelector(`[data-payment-id="${CSS.escape(String(p.id))}"]`))return;
  const row=document.createElement('article');row.className='rqHistRow rqPaymentReceiptRow';row.dataset.paymentId=p.id;
  row.innerHTML=`<div><div class="rqHistType">OFFICIAL RECEIPT</div><div class="rqHistNo">${esc(p.receipt_number)}</div></div><div class="rqHistDate">${esc(dateFmt(p.paid_at))}</div><div class="rqHistCustomer">${esc(p.customer_name||'Customer')}<br><span class="rqDocMeta">${esc(p.invoice_number||p.order_no||'')}</span><div class="rqPaymentMeta"><span>${esc(typeLabel(p.payment_type))}</span><span><b>${money(p.amount_minor)}</b></span></div></div><div><span class="rqDocBadge paid">PAID</span></div><div class="rqHistActions"><button class="btn" type="button" data-open-payment="${esc(p.id)}">Buka</button></div>`;
  host.prepend(row);
 });
