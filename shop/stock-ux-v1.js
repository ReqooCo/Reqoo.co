...
let stockMap=new Map(),selectedProduct='',selectedVariant=0,observer=null,busy=false;
...
function markCard(btn,p){if(!btn||!p)return;if(soldOut(p)){btn.disabled=true;btn.textContent='Stok Habis';btn.setAttribute('aria-disabled','true');btn.style.opacity='.48';btn.style.cursor='not-allowed'}}
function refreshCards(){document.querySelectorAll('[data-product]').forEach(btn=>{const p=product(btn.dataset.product);if(p)markCard(btn,p)})}
function refreshModal(){const modal=document.getElementById('modal');if(!modal?.classList.contains('open')||!selectedProduct)return;const p=product(selectedProduct);if(!p)return;const vars=Array.isArray(p.variants)&&p.variants.length?p.variants:(Number(p.basePrice||0)>0?[{name:'Standard',price:Number(p.basePrice),stock:null}]:[]);modal.querySelectorAll('[data-v]').forEach(btn=>{const i=Number(btn.dataset.v),s=variantStock({variants:vars},i),disabled=s!==null&&s<=0;btn.disabled=disabled;btn.style.opacity=disabled?'.42':'';btn.style.cursor=disabled?'not-allowed':'';const small=btn.querySelector('small');if(small&&!small.dataset.reqooOriginal)small.dataset.reqooOriginal=small.textContent;if(disabled&&small)small.textContent='Stok Habis';else if(small&&small.dataset.reqooOriginal)small.textContent=small.dataset.reqooOriginal});const q=modal.querySelector('#detailQty'),add=modal.querySelector('#addToCart'),s=variantStock({variants:vars},selectedVariant);if(q){q.min='1';if(s!==null){q.max=String(Math.max(1,s));if(Number(q.value)>s)q.value=String(Math.max(1,s));if(s<=0)q.value='1'}else q.removeAttribute('max')}if(add){const blocked=s!==null&&s<=0;add.disabled=blocked;add.style.opacity=blocked?'.48':'';add.style.cursor=blocked?'not-allowed':'';add.textContent=blocked?'Stok Habis':'Tambah ke Cart'}}
function refresh(){if(busy)return;busy=true;try{observer?.disconnect();refreshCards();refreshModal()}finally{busy=false;observer?.observe(document.documentElement,{childList:true,subtree:true})}}
...
observer=new MutationObserver(()=>{if(!busy)refresh()});
