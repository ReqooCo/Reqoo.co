...
import { onRequest as handleShopAdminFlow } from './shop-admin-flow-v2.js';
...
async function shopAdminCompat(request,env){if(request.method!=='POST')return handleShopAdmin({request,env});try{const d=await request.clone().json();if(String(d.action)==='status'&&String(d.status||'').toLowerCase()==='completed')d.status='fulfilled';return handleShopAdmin({request:new Request(request.url,{method:'POST',headers:request.headers,body:JSON.stringify(d)}),env})}catch{return handleShopAdmin({request,env})}}
...
export default {async fetch(request,env,ctx){const url=new URL(request.url),path=url.pathname,bridgedEnv=withAdminEnv(env);if(path==='/api/shop')return handleShopFlow({request,env});if(path==='/api/shop-admin')return handleShopAdminFlow({request,env});if(path==='/api/billplz')return secureShop(request,bridgedEnv);if(path==='/api/pksk')return handlePksk(request,bridgedEnv);if(path==='/api/pksk-payment-v2')return handlePkskPaymentV2({request,env:bridgedEnv});if(path==='/api/shop-hero')return handleHero(request,bridgedEnv);if(path==='/api/core')return handleCore({request,env:bridgedEnv});if(path==='/api/sim-admin'||path==='/api/sim-admin-v2')return handleSimAdmin({request,env});if(path==='/api/product-image')return handleProductImage({request,env:bridgedEnv,ctx});if(path==='/api/invoice-v1'||path.startsWith('/api/invoice-v1/'))return handleInvoice({request,env:bridgedEnv});if(path==='/api/license-v1'||path.startsWith('/api/license-v1/'))return handleLicense({request,env:bridgedEnv});if(path==='/api/health')return new Response(JSON.stringify({ok:!!env.DB,service:'reqoo-api',db:!!env.DB,media:!!env.MEDIA}),{status:env.DB?200:503,headers:{'content-type':'application/json; charset=UTF-8','cache-control':'no-store','access-control-allow-origin':'*'}});return new Response(JSON.stringify({ok:false,error:'Not found'}),{status:404,headers:{'content-type':'application/json; charset=UTF-8','cache-control':'no-store','access-control-allow-origin':'*'}})}};
