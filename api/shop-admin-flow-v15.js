...
async function dashboardSummary(env){
  ...
  const today=myDate(),invoiceDue=invoiceRows.map(r=>{const balance=Math.max(0,Number(r.total_minor||0)-Number(r.paid_minor||0)),days=daysBetween(today,r.due_at);return{...r,balance_minor:balance,days_to_due:days}}).filter(r=>r.balance_minor>0&&r.days_to_due!==null);
