...
async function dashboardSummary(env){
  const [kpi,repeat,due,trend,latest,pending]=await Promise.all([
...
export async function onRequest({request,env}){if(request.method==='OPTIONS')return new Response(null,{status:204,headers:C});const d=await data(request),action=S(d.action);if(action==='dashboardSummary'||action==='productsDashboard'){if(!auth(request,env,d))return J({ok:false,error:'Unauthorized'},401);if(!env.DB)return J({ok:false,error:'D1 binding DB tidak dijumpai'},503);try{return action==='dashboardSummary'?dashboardSummary(env):productsDashboard(env)}catch(e){console.error('REQOO admin v15:',e);return J({ok:false,error:e?.message||String(e)},500)}}return legacy({request,env});}