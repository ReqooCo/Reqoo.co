...
function meta(raw) {
  ...
  if (!text.startsWith(prefix)) return { options: [] };
  try {
    const bin = atob(text.slice(prefix.length));
    const decoded = decodeURIComponent(escape(bin));
    const value = JSON.parse(decoded);
    return { options: Array.isArray(value?.options) ? value.options : [] };
  } catch { return { options: [] }; }
}
function cleanText(v, max = 1000) { return String(v ?? '').trim().slice(0, max); }
function chosen(v) { return v && typeof v === 'object' ? String(v.value ?? v.label ?? '') : String(v ?? ''); }
function moneyCents(v) { const n = Number(v); return Number.isFinite(n) && n >= 0 ? Math.round(n * 100) : NaN; }
...
function priceProduct(product, item) {
  ...
  const customization = item.customization && typeof item.customization === 'object' ? item.customization : {};
  const addons = Array.isArray(item.addons) ? item.addons : [];
  let unit = Number(product.sale_price_minor ?? product.base_price_minor ?? 0);
  if (!Number.isInteger(unit) || unit < 0) throw new Error(`Harga product tidak sah: ${product.id}`);

  for (const opt of options.filter(x => x.group === 'variation')) {
    const raw = variation[opt.id] ?? variation[opt.label];
    const value = chosen(raw);
    if (opt.required && !value) throw new Error(`Pilihan ${opt.label} diperlukan`);
    if (!value) continue;
    const match = (opt.values || []).find(v => String(v.value ?? v.label) === value);
  ...
  for (const opt of options.filter(x => x.group === 'addon')) {
    const selected = selectedAddon(addons, opt);
    if (!selected) {
      if (opt.required) throw new Error(`Add-on ${opt.label} diperlukan`);
      continue;
    }
    let values = selected.values ?? selected.value ?? selected.label ?? true;
    if (!Array.isArray(values)) values = [values];
    if (!values.length && opt.required) throw new Error(`Add-on ${opt.label} diperlukan`);
    for (const raw of values) {
      const label = raw && typeof raw === 'object' ? String(raw.label ?? raw.value ?? '') : String(raw);
      if (opt.field_type === 'select') {
        const match = (opt.values || []).find(v => String(v.label) === label || String(v.value ?? '') === label);
        if (!match) throw new Error(`Pilihan add-on tidak sah: ${opt.label}`);
        const cents = moneyCents(match.price);
        if (!Number.isInteger(cents)) throw new Error(`Harga add-on tidak sah: ${opt.label}`);
        unit += cents;
      } else {
        const match = (opt.values || []).find(v => String(v.label) === label || String(v.value ?? '') === label);
        if ((opt.values || []).length && !match) throw new Error(`Pilihan add-on tidak sah: ${opt.label}`);
        const cents = match && match.price !== undefined ? moneyCents(match.price) : moneyCents(opt.addonPrice || 0);
        if (!Number.isInteger(cents)) throw new Error(`Harga add-on tidak sah: ${opt.label}`);
        unit += cents;
      }
    }
  ...
  for (const opt of options.filter(x => x.group === 'custom')) {
    const raw = customization[opt.id] ?? customization[opt.label] ?? '';
    const value = raw && typeof raw === 'object' ? raw.value ?? '' : raw;
    if (opt.required && !String(value).trim()) throw new Error(`Ruangan ${opt.label} diperlukan`);
  }
  return { unit, variation, customization, addons };
}
...
export async function createOrder(request, env) {
  ...
  if (idempotencyKey.length < 16) return json({ error: 'Idempotency-Key diperlukan' }, 400, origin);

  let input;
  try { input = await request.json(); } catch { return json({ error: 'Data pesanan tidak sah' }, 400, origin); }
  ...
  if (!name || !phone || !shippingAddress) return json({ error: 'Nama, telefon dan alamat diperlukan' }, 400, origin);
  if (!Array.isArray(input?.items) || !input.items.length) return json({ error: 'Cart kosong' }, 400, origin);
  ...
  const requested = input.items.map(x => ({
    productId: cleanText(x?.product_id || x?.id, 100),
    quantity: Math.max(1, Math.min(999, Math.floor(Number(x?.quantity || 1)))),
    customization: x?.customization && typeof x.customization === 'object' ? x.customization : {},
    variation: x?.variation && typeof x.variation === 'object' ? x.variation : {},
    addons: Array.isArray(x?.addons) ? x.addons : []
  }));
  const ids = [...new Set(requested.map(x => x.productId).filter(Boolean))];
  if (ids.length !== requested.length) return json({ error: 'Product ID tidak sah' }, 400, origin);
  const placeholders = ids.map(() => '?').join(',');
  const { results } = await env.DB.prepare(`SELECT id,sku,name,base_price_minor,sale_price_minor,status,internal_notes FROM products WHERE id IN (${placeholders})`).bind(...ids).all();
  ...
  for (const item of requested) {
    ...
    if (!product || product.status !== 'active') return json({ error: `Product tidak tersedia: ${item.productId}` }, 409, origin);
    const priced = priceProduct(product, item);
    const line = priced.unit * item.quantity;
    if (!Number.isSafeInteger(line)) return json({ error: 'Jumlah pesanan terlalu besar' }, 400, origin);
    subtotal += line;
    if (!Number.isSafeInteger(subtotal)) return json({ error: 'Jumlah pesanan terlalu besar' }, 400, origin);
    lines.push({ product, ...item, ...priced, line });
  }
