export async function onRequest(context){
  ...
  if(host==='pksk.sim.reqoo.co'&&!url.pathname.startsWith('/api/')){
    ...
    if(isSimulator)return new HTMLRewriter().on('head',{element(el){el.append('<script src="/simulator/js/v65-writing-overlay-fix.js?v=66"></script>',{html:true})}}).transform(response);
  ...
  if(host==='reqoo.co'&&isSimulator)return new HTMLRewriter().on('head',{element(el){el.append('<script src="/simulator/js/v65-writing-overlay-fix.js?v=66"></script>',{html:true})}}).on('body',{element(el){el.append('<script src="/shop/payment-fallback.js?v=4"></script><script src="/shop/payment-fix.js?v=1"></script>',{html:true})}}).transform(response);
