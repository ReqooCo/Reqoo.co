export async function onRequest(context){
  ...
  if(host==='pksk.sim.reqoo.co'&&!url.pathname.startsWith('/api/')){
    ...
    const isSimulator=path==='/simulator'||path.startsWith('/simulator/');const isAccess=path==='/access'||path.startsWith('/access/');
    url.pathname=path==='/'?'/sim/pksk/':`/sim/pksk${path}`;const response=await context.env.ASSETS.fetch(url);
    if(!isSimulator&&!isAccess)return response;const type=response.headers.get('content-type')||'';if(!type.includes('text/html'))return response;
    return new HTMLRewriter().on('head',{element(el){if(isSimulator)el.append('<script src="/simulator/js/v58-core.js?v=58"></script>',{html:true})}}).transform(response);
  }
