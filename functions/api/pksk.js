...
async function getPaymentProof(d,env){const orderNo=String(d.orderNo||'').trim();if(!orderNo)return {ok:false,error:'Order diperlukan'};const order=await env.DB.prepare('SELECT payment_ref FROM orders WHERE id=?').bind(orderNo).first();if(!order?.payment_ref)return {ok:false,error:'Bukti pembayaran tidak dijumpai'};const obj=await env.PROOFS.get(String(order.payment_ref));if(!obj)return {ok:false,error:'Fail bukti pembayaran tidak dijumpai dalam R2'};const mime=obj.httpMetadata?.contentType||'image/jpeg';const bytes=new Uint8Array(await obj.arrayBuffer());let binary='';const chunk=0x8000;for(let i=0;i<bytes.length;i+=chunk){binary+=String.fromCharCode(...bytes.subarray(i,i+chunk));}return {ok:true,orderNo,dataUrl:`data:${mime};base64,${btoa(binary)}`};}
