...
const preview=await worker.fetch(new Request('https://reqoo.co/d/abc123'),{ASSETS:{fetch:async()=>new Response(publicHtml,{headers:{'content-type':'text/html;charset=UTF-8'}})}});
const previewBody=await preview.text();
assert.equal(preview.headers.get('x-reqoo-document-preview'),'off');
assert.match(previewBody,/REQOO\.CO — Document/);
assert.doesNotMatch(previewBody,/property="og:image"/,'Document links must not force a generic image preview');
assert.doesNotMatch(previewBody,/property="og:title"/,'Document links should rely on the WhatsApp message rather than a social card');
