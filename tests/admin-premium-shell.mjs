...
assert.match(shell,/\['overview','orders','production','products','customers','documents','finance'\]/,'mobile dock must include Documents');
...
assert.match(css,/repeat\(5,1fr\)/,'mobile dock must use a five-column two-row layout for seven destinations');
assert.match(css,/padding-bottom:132px/,'mobile content must clear the two-row dock');
assert.match(css,/@media\(max-width:900px\)/,'Admin shell must be responsive');
console.log('PASS: premium Admin shell stays isolated from Shop Admin safe mode and exposes Documents on mobile.');
