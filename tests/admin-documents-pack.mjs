...
assert.match(html,/quotation/i);assert.match(html,/invoice/i);assert.match(html,/receipt/i);assert.match(html,/documents\.js\?v=15/);
assert.match(js,/ordersDashboard/);assert.doesNotMatch(js,/listOrders/);assert.match(js,/initialDocOrder/);assert.match(js,/URLSearchParams\(location\.search\)/);assert.match(js,/limit:80/);assert.match(js,/loadMoreDocOrders/);assert.match(js,/setTimeout\(\(\)=>loadOrders\(true\),260\)/);assert.match(js,/listDocuments',\{orderId:initialDocOrder,limit:50\}/);assert.doesNotMatch(js,/X-Admin-Token/);assert.doesNotMatch(js,/localStorage\.getItem/);assert.match(js,/listDocuments/);assert.match(js,/Pautan rasmi REQOO\.CO/);assert.doesNotMatch(js,/\?v='\+rev/,'WhatsApp document URL should stay as short as possible');assert.match(js,/rqPaperPaymentTerms/);assert.match(js,/normalizeBankDetails/);assert.doesNotMatch(js,/MAYBANK\\\\\\\\nAccount Name/);assert.match(js,/createDocument/);assert.match(js,/saveDocumentSettings/);assert.match(js,/w\.print\(\)/);
