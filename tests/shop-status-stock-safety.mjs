...
const flow=fs.readFileSync(new URL('../api/shop-admin-flow-v7.js',import.meta.url),'utf8');assert.match(flow,/action==='status'/);assert.match(flow,/status==='failed'\|\|status==='cancelled'/);assert.match(flow,/action:'rejectPayment'/);assert.match(flow,/payment_status\)\.toLowerCase\(\)==='paid'/);
...
const v15=fs.readFileSync(new URL('../api/shop-admin-flow-v15.js',import.meta.url),'utf8');assert.match(v15,/shop-admin-flow-v14\.js/,'v15 must preserve v14 and every prior safety layer');
const worker=fs.readFileSync(new URL('../api/worker.js',import.meta.url),'utf8');assert.match(worker,/shop-admin-flow-v15\.js/,'worker must route Shop admin through v15');
