...
assert.equal(rows[0].url,'/api/product-image?key=products/prd_1/old.webp');
assert.equal(rows[0].is_cover,1,'existing cover should remain cover when adding images');
assert.equal(rows[1].url,'/api/product-image?key=products/prd_1/new.webp');
assert.equal(rows[1].is_cover,0);
console.log('PASS: Shop Admin image upload appends safely without deleting existing gallery images.');
