...
async function injectDocumentPreview(response){
  ...
  headers.set('x-reqoo-document-preview','off');
...
async function injectShopRuntime(response,force=false){
  ...
  const runtime=isPublicShop?'<script src="/shop/shop-core-v1.js?v=7"></script><script src="/shop/shop-inventory-v1.js?v=1"></script><script src="/shop/postpurchase-v1.js?v=1"></script>':isAccount?'<script src="/shop/account-premium-v1.js?v=1"></script>':'<script src="/shop/shop-core-v1.js?v=7"></script>';
...
export default{
  async fetch(request,env,ctx){
    ...
    if(host==='reqoo.co'&&/^\/d\/[A-Za-z0-9-]+\/?$/.test(url.pathname)){
      ...
      return injectDocumentPreview(response);
    }
