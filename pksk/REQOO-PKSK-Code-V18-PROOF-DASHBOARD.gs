...
const MAX_DEVICES=2;
const PROOF_FOLDER_NAME='PKSK_Payment_Proofs';
...
function setupPKSK(){
  pkskSheet_(ORDERS_SHEET,['orderNo','name','phone','ref','amount','status','licenseCode','createdAt','verifiedAt','adminNote','proofUrl','proofName','proofDriveId']);
...
function createOrder(d){
  const sh=pkskSheet_(ORDERS_SHEET,['orderNo','name','phone','ref','amount','status','licenseCode','createdAt','verifiedAt','adminNote','proofUrl','proofName','proofDriveId']);
  const orderNo=String(d.orderNo||'').trim().toUpperCase(),name=String(d.name||'').trim(),phone=String(d.phone||'').trim(),ref=String(d.ref||'').trim();
  if(!orderNo||!name||!phone)return {ok:false,error:'Maklumat tidak lengkap'};
  const rows=sh.getDataRange().getValues();
  for(let i=1;i<rows.length;i++){
    if(String(rows[i][0])===orderNo){
      if(!String(rows[i][12]||'') && d.proofData){const proof=saveProof_(d,orderNo);sh.getRange(i+1,11,1,3).setValues([[proof.url,proof.name,proof.id]]);}
      return {ok:true,orderNo,duplicate:true,proofUploaded:Boolean(String(sh.getRange(i+1,13).getValue()||''))};
    }
  }
  if(!d.proofData)return {ok:false,error:'Bukti pembayaran diperlukan'};
  const proof=saveProof_(d,orderNo);
  sh.appendRow([orderNo,new Date(),name,phone,ref,PKSK_AMOUNT,'PENDING','',new Date(),'',''+proof.url,proof.name,proof.id]);
  return {ok:true,orderNo,status:'PENDING',proofUploaded:true};
}
function saveProof_(d,orderNo){
  const dataUrl=String(d.proofData||'');
  const m=dataUrl.match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/);
  if(!m)throw new Error('Format bukti mesti JPG, PNG atau WEBP.');
  const mime=m[1],base64=m[2],estimated=Math.floor(base64.length*3/4);
  if(estimated>MAX_PROOF_BYTES)throw new Error('Saiz bukti terlalu besar. Maksimum 6MB.');
  const bytes=Utilities.base64Decode(base64);if(bytes.length>MAX_PROOF_BYTES)throw new Error('Saiz bukti terlalu besar. Maksimum 6MB.');
  const ext=mime==='image/png'?'png':mime==='image/webp'?'webp':'jpg';
  const safeName=String(d.proofName||('bukti-'+orderNo+'.'+ext)).replace(/[^a-zA-Z0-9._-]/g,'_').slice(-120);
  const it=DriveApp.getFoldersByName(PROOF_FOLDER_NAME);const folder=it.hasNext()?it.next():DriveApp.createFolder(PROOF_FOLDER_NAME);
  const file=folder.createFile(Utilities.newBlob(bytes,mime,safeName));
  return {url:file.getUrl(),name:file.getName(),id:file.getId()};
}
function verifyOrder(d){const sh=pkskSheet_(ORDERS_SHEET,['orderNo','name','phone','ref','amount','status','licenseCode','createdAt','verifiedAt','adminNote','proofUrl','proofName','proofDriveId']);const lic=pkskSheet_(LICENSES_SHEET,['licenseCode','orderNo','name','phone','status','createdAt','activatedAt']);const v=sh.getDataRange().getValues();for(let i=1;i<v.length;i++)if(String(v[i][0])===String(d.orderNo)){let code=String(v[i][6]||''),now=new Date();if(!code){code=pkskMakeCode_();lic.appendRow([code,d.orderNo,String(v[i][1]||''),String(v[i][2]||''),'ACTIVE',v[i][7]||now,now])}sh.getRange(i+1,6,1,4).setValues([['PAID',code,'ACTIVE',now]]);pkskLog_(code,'license_activated','Bayaran disahkan dan lesen diaktifkan.','');return {ok:true,accessCode:code,status:'PAID'}}return {ok:false,error:'Order tidak dijumpai'}}
function rejectOrder(d){const sh=pkskSheet_(ORDERS_SHEET,['orderNo','name','phone','ref','amount','status','licenseCode','createdAt','verifiedAt','adminNote','proofUrl','proofName','proofDriveId']);const row=pkskFindRow_(sh,1,d.orderNo);if(row<2)return {ok:false,error:'Order tidak dijumpai'};sh.getRange(row,6).setValue('REJECTED');const code=String(sh.getRange(row,7).getValue()||'');if(code){const lr=pkskLicenseRow_(code);if(lr>1)pkskSheet_(LICENSES_SHEET,['licenseCode','orderNo','name','phone','status','createdAt','activatedAt']).getRange(lr,5).setValue('BLOCKED');pkskLog_(code,'license_blocked','Order ditolak.','')}return {ok:true}}
function listOrders(){
  const sh=pkskSheet_(ORDERS_SHEET,['orderNo','name','phone','ref','amount','status','licenseCode','createdAt','verifiedAt','adminNote','proofUrl','proofName','proofDriveId']);
  const v=sh.getDataRange().getValues();
  return {ok:true,orders:v.length<2?[]:v.slice(1).reverse().map(r=>{const code=String(r[6]||'');return {orderNo:String(r[0]||''),name:String(r[1]||''),phone:String(r[2]||''),ref:String(r[3]||''),amount:Number(r[4]||29),status:String(r[5]||''),accessCode:code,licenseStatus:code&&pkskLicense_(code)?pkskLicense_(code).status:'',deviceCount:code?pkskDevices_(code).length:0,progressCount:code?pkskProgress_(code).filter(x=>x.completed).length:0,createdAt:r[7] instanceof Date?r[7].toISOString():String(r[7]||''),verifiedAt:r[8] instanceof Date?r[8].toISOString():String(r[8]||''),proofUrl:String(r[10]||''),proofName:String(r[11]||''),proofDriveId:String(r[12]||'')}})}}
