...
def main():
    if len(sys.argv)!=2 or not sys.argv[1].isdigit(): raise SystemExit('usage: validate_pksk_b_science_new_batch.py <batch-number>')
    b=int(sys.argv[1]); tag=f'{b:03d}'; errors=[]; warnings=[]
    mp=CUR/f'science_new_batch_{tag}_manifest.json'
    ...
    expected=m.get('expectedAnswers') or {}; fam_expected=m.get('familyCounts') or {}; ans_expected={int(k):int(v) for k,v in (m.get('answerPositionCounts') or {}).items()}; expected_visual_count=int(m.get('expectedVisualCount',0))
    source=[x for x in read_jsonl(SURVIVORS) if x.get('domain')=='Sains']
    if len(source)!=124: errors.append(f'strict_source_science_count:{len(source)}!=124')
    prev=[]
    for n in range(1,b):
        p=OUT/f'b_science_new_batch_{n:03d}_validated.jsonl'
        if not p.exists(): errors.append(f'missing_previous_validated:{p.name}'); continue
        prev.extend(read_jsonl(p))
    if len(prev)!=start-1: errors.append(f'previous_validated_science_count:{len(prev)}!={start-1}')
    if prev and {x.get('bankId') for x in prev}!={f'B-SCI-NEW-{i:04d}' for i in range(1,start)}: errors.append('previous_science_ids_not_contiguous')
    if any(x.get('domain')!='Sains' or x.get('reviewStatus')!='EDITORIAL_QA_PASS' for x in prev): errors.append('previous_validated_science_baseline_invalid')
    files=sorted(CUR.glob(f'science_new_batch_{tag}[a-z].jsonl')); batch=[]
    for p in files: batch.extend(read_jsonl(p))
    ids=[str(x.get('bankId') or '') for x in batch]; expected_ids={f'B-SCI-NEW-{i:04d}' for i in range(start,end+1)}
    ...
    fam=Counter(); ans=Counter(); constructs=Counter(); sigs=Counter(); exact=defaultdict(list); nums=defaultdict(list); diffs=[]; visuals=Counter(); validated=[]
    for x in batch:
        ...
        if x.get('domain')!='Sains': errors.append(f'domain_invalid:{bid}')
        ...
        if c in prev_constructs: errors.append(f'construct_reused_from_previous_science:{bid}:{c}')
        if s in prev_sigs: errors.append(f'pattern_signature_reused_from_previous_science:{bid}:{s}')
        ...
        vis=x.get('visual')
        if vis is not None:
            if not isinstance(vis,dict): errors.append(f'visual_invalid:{bid}')
            else:
                ...
                leaked=FORBIDDEN_VISUAL_KEYS & set(vis)
                if leaked: errors.append(f'visual_answer_leak:{bid}:{sorted(leaked)}')
        n0=norm(q); nn=norm(q,True)
        if n0 in exact_base: errors.append(f'exact_collision_with_existing_science:{bid}:{exact_base[n0]}')
        if nn in num_base: errors.append(f'number_template_collision_with_existing_science:{bid}:{num_base[nn]}')
        exact[n0].append(bid); nums[nn].append(bid)
        for sid,sn in near_base:
            if sn and SequenceMatcher(None,nn,sn).ratio()>=0.90: warnings.append(f'near_existing_science:{bid}:{sid}')
        y=dict(x); y['reviewStatus']='EDITORIAL_QA_PASS'; y['qaNotes']='Independent answer, Science Year 6, structured-visual, duplicate and editorial gates passed; final cross-bank QA still required.'; y['validationBatch']=f'B_SCIENCE_NEW_BATCH_{tag}'; validated.append(y)
    ...
    if sum(visuals.values())!=expected_visual_count: errors.append(f'visual_count:{sum(visuals.values())}!={expected_visual_count}')
    ...
    for i,a in enumerate(batch):
        ...
        for b2 in batch[i+1:]:
            ...
            if na and nb and SequenceMatcher(None,na,nb).ratio()>=0.90: warnings.append(f'near_internal_science:{a.get("bankId")}:{b2.get("bankId")}')
    ready_after=124+len(prev)+count
    report={'version':f'B_SCIENCE_NEW_BATCH_{tag}_QA_V1','batchCount':len(batch),'strictSourceScienceReady':len(source),'previousValidatedScienceReady':len(prev),'existingScienceBaselineCount':len(baseline),'uniqueConstructs':len(constructs),'familyCounts':dict(fam),'answerPositionCounts':{str(i):ans.get(i,0) for i in range(4)},'visualCount':sum(visuals.values()),'visualKinds':dict(visuals),'difficulty':{'min':min(diffs) if diffs else None,'max':max(diffs) if diffs else None,'average':round(sum(diffs)/len(diffs),3) if diffs else None},'independentExpectedAnswerCount':len(expected),'expectedScienceReadyAfterPlan':ready_after,'expectedScienceRemainingAfterPlan':400-ready_after,'internalExactDuplicateGroups':ie,'internalNumberNormalizedDuplicateGroups':inn,'errors':errors,'warnings':warnings,'pass':not errors and not warnings}
    rp=OUT/f'b_science_new_batch_{tag}_qa.json'; op=OUT/f'b_science_new_batch_{tag}_validated.jsonl'; rp.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    if report['pass']:
