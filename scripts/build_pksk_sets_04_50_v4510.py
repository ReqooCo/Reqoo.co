...
def leak(o,a):
    r=[len(n(x)) for i,x in enumerate(o) if i!=a]
    w=[len(n(x).split()) for i,x in enumerate(o) if i!=a]
...
def pos(o,a):
    x=o[a]; r=[v for j,v in enumerate(o) if j!=a]; return r
A=[
...
('EQ','Kamu menerima kritikan terhadap hasil kerja yang kamu banggakan. Apakah tindakan terbaik?',['Marah kepada pemberi kritikan','Nilai kritikan dan ambil perkara yang boleh diperbaiki','Abaikan semua komen','Berhenti daripada projek'],'resilience'),
...
('EQ','Kamu gagal mencapai sasaran walaupun sudah berusaha. Apakah tindak balas paling konstruktif?',['Anggap usaha tidak berguna','Kenal pasti punca dan ubah strategi','Salahkan keadaan','Berhenti mencuba'],'growth')]
AG=[
...
def makeA(s):
    ...
    for i in range(30):
        if i%3==0:
            q,o,f=AG[(i//3+s)%10]; a=(i+s)%4; o=[o[a]]+[] if False else o; # preserve bank answer at 0
            o=pos(o,0); o.insert(a,AG[(i//3+s)%10][1][0]); out.append(item(s,'BAHAGIAN A','Setuju/Tidak Setuju',q+' '+ctx[i%10]+'.',o,a,f,'AGREE_DISAGREE',i+1))
        else:
            typ,q,o,f=A[(i+s)%10]; a=(i*3+s)%4; right=o[1]; rest=[x for j,x in enumerate(o) if j!=1]; rest.insert(a,right); out.append(item(s,'BAHAGIAN A',typ,q+' '+ctx[i%10]+'. Apakah tindakan paling wajar?',rest,a,f,'SITUATIONAL',i+1))
    return out
B_BANK=[
...
def makeB(s):
    ...
    for i in range(70):
        ...
        if typ=='Matematik':
            a=12+(s+i*3)%25; b=[2,3,4,5,6][(s+i)%5]; ansv=data(a,b); opts=[str(ansv-b),str(ansv),str(ansv+b),str(ansv+2*b)]; right=1
        else: opts=data; right=0
        a=(i+s)%4; r=[x for j,x in enumerate(opts) if j!=right]; r.insert(a,opts[right]); out.append(item(s,'BAHAGIAN B',typ,q.format(a=12+(s+i*3)%25,b=[2,3,4,5,6][(s+i)%5]) if '{' in q else q,r,a,f,'MCQ',i+1))
    return out
def makeC(s):
...
def build(s):
    q=makeA(s)+makeB(s); c=makeC(s); assert len(q)==100 and len(c)==3
    assert len({x['id'] for x in q})==100 and len({x['question'].casefold() for x in q})==100
    prior=set()
    for k in range(1,s):
    ...
    A=[x for x in q if x['section']=='BAHAGIAN A']; pc=Counter(x['answerIndex'] for x in A); fc=Counter(x['format'] for x in A)
    assert len(pc)==4 and max(pc.values())<=8 and fc['AGREE_DISAGREE']>=10
