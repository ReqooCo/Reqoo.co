...
def main():
    if len(sys.argv)!=2 or not sys.argv[1].isdigit(): raise SystemExit('usage: validate_pksk_b_english_new_batch.py <batch-number>')
    b=int(sys.argv[1]); tag=f'{b:03d}'; errors=[]; warnings=[]
    mp=CUR/f'english_new_batch_{tag}_manifest.json'
    ...
    source=[x for x in read_jsonl(SURVIVORS) if x.get('domain')=='English']
    if len(source)!=106: errors.append(f'strict_source_english_count:{len(source)}!=106')
    prev=[]
    for n in range(1,b):
        p=OUT/f'b_english_new_batch_{n:03d}_validated.jsonl'
        if not p.exists(): errors.append(f'missing_previous_validated:{p.name}'); continue
        prev.extend(read_jsonl(p))
    if len(prev)!=start-1: errors.append(f'previous_validated_english_count:{len(prev)}!={start-1}')
    if prev and {x.get('bankId') for x in prev}!={f'B-EN-NEW-{i:04d}' for i in range(1,start)}: errors.append('previous_english_ids_not_contiguous')
    if any(x.get('domain')!='English' or x.get('reviewStatus')!='EDITORIAL_QA_PASS' for x in prev): errors.append('previous_validated_english_baseline_invalid')
    files=sorted(CUR.glob(f'english_new_batch_{tag}[a-z].jsonl')); batch=[]
    for p in files: batch.extend(read_jsonl(p))
    ids=[str(x.get('bankId') or '') for x in batch]; expected_ids={f'B-EN-NEW-{i:04d}' for i in range(start,end+1)}
    ...
    for x in batch:
        ...
        if x.get('domain')!='English': errors.append(f'domain_invalid:{bid}')
        ...
        if c in prev_constructs: errors.append(f'construct_reused_from_previous_english:{bid}:{c}')
        if s in prev_sigs: errors.append(f'pattern_signature_reused_from_previous_english:{bid}:{s}')
        ...
        if n0 in exact_base: errors.append(f'exact_collision_with_existing_english:{bid}:{exact_base[n0]}')
        if nn in num_base: errors.append(f'number_template_collision_with_existing_english:{bid}:{num_base[nn]}')
        exact[n0].append(bid); nums[nn].append(bid)
        for sid,sn in near_base:
            if sn and SequenceMatcher(None,nn,sn).ratio()>=0.90: warnings.append(f'near_existing_english:{bid}:{sid}')
        y=dict(x); y['reviewStatus']='EDITORIAL_QA_PASS'; y['qaNotes']='Independent answer, English Year 6, duplicate and editorial gates passed; final cross-bank QA still required.'; y['validationBatch']=f'B_ENGLISH_NEW_BATCH_{tag}'; validated.append(y)
    ...
    for i,a in enumerate(batch):
        ...
        for b2 in batch[i+1:]:
            ...
            if na and nb and SequenceMatcher(None,na,nb).ratio()>=0.90: warnings.append(f'near_internal_english:{a.get("bankId")}:{b2.get("bankId")}')
    ready_after=106+len(prev)+count
    report={'version':f'B_ENGLISH_NEW_BATCH_{tag}_QA_V1','batchCount':len(batch),'strictSourceEnglishReady':len(source),'previousValidatedEnglishReady':len(prev),'existingEnglishBaselineCount':len(baseline),'uniqueConstructs':len(constructs),'familyCounts':dict(fam),'answerPositionCounts':{str(i):ans.get(i,0) for i in range(4)},'difficulty':{'min':min(diffs) if diffs else None,'max':max(diffs) if diffs else None,'average':round(sum(diffs)/len(diffs),3) if diffs else None},'independentExpectedAnswerCount':len(expected),'expectedEnglishReadyAfterPlan':ready_after,'expectedEnglishRemainingAfterPlan':400-ready_after,'internalExactDuplicateGroups':ie,'internalNumberNormalizedDuplicateGroups':inn,'errors':errors,'warnings':warnings,'pass':not errors and not warnings}
    rp=OUT/f'b_english_new_batch_{tag}_qa.json'; op=OUT/f'b_english_new_batch_{tag}_validated.jsonl'; rp.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    if report['pass']:
